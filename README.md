...
- アプリ起動時に、同一の`location.host`（ドメイン＋ポート）を持つフィードが複数あると、最初に追加されたもの1件だけを残して残りを自動的に「取得しない」に一時停止します（Tampermonkeyが同じサイトの別ページで別のフィードリンクを拾ってしまった場合などの重複対策）。フィード名を右クリック／長押しで一時停止・再開を手動操作したフィードは、以後この自動整理の対象から外れます（他端末の同期にも反映されます）。
