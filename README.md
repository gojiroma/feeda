...
- 画面幅が600px以下（小さめのスマートフォン）になると、3ペインではなく1カラムのレイアウトに切り替わります。フィード一覧は上部の横スクロールできるチップになり、タップで即座に切り替えられます（階層を行き来する必要はありません）。記事は一覧上でタップするとその場でアコーディオン式に本文が展開され、別画面のプレビューには遷移しません。
