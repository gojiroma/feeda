...
feedaはPWA（インストール可能なWebアプリ）にも対応しています。デスクトップ/モバイル問わずブラウザのアドレスバーやメニューから「インストール」するとホーム画面・アプリ一覧にアイコンが追加され、単独のウィンドウで起動できます。`sw.js`がCSS/JS/アイコンなどの静的ファイルだけをキャッシュするため、オフラインや電波の弱い状態でもアプリの起動自体は速くなります（`/api/`宛のリクエスト＝フィード取得・同期は対象外で、常にネットワークへ直接アクセスします）。
